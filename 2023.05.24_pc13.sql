...
(  buyer_id           CHAR(6)       NOT NULL,   --거래처코드 
 ...
 ALTER TABLE buyer
   add ( CONSTRAINT pk_buyer PRIMARY KEY(buyer_id),
           CONSTRAINT fr_buyer_lgu  foreign key(buyer_lgu) --LGU: 대구분 코드
...
ALTER TABLE EMP ADD (REMARK varchar2(4000));   -- 필드삽입
...
<1번> 덧셈연산자를 이용하여 모든 사원에 대해서 연봉에 대해 300만원의 급여 인상을 계산한 후,
    사원의 사번, 이름, 급여, 인상된 연봉을 출력하시오,

SELECT EMPNO 사번, ENAME 이름, SAL 급여, (SAL*12)+300 "인상된 연봉" --? 맞나?
FROM EMP;

<2번> 사원번호 7999사람의 업무(JOB)를 '프로그래머'로 수정하시오.
...
<3번> EMP 테이블 JOB 데이터 타입을 VARCHAR2(20)으로 수정하시오.
ALTER TABLE EMP
MODIFY (JOB varchar2(20));
--테이블을 수정 -> DDL

<4번> EMP 테이블에 사원번호로 DEPT 테이블과 외래키를 생성하시오.

ALTER TABLE EMP
    ADD (CONSTRAINT XFK_EMP Foreign key (DEPTNO) -- 키 만들 때 
                            REFERENCES DEPT(DEPTNO));

<5번> 사원의 이름, 급여, 연간 총 수입을 총 수입이 많은 것부터 작은 순으로 출력하시오,
연간 총수입은 월급에 12를 곱한 후 $100의 상여금을 더해서 계산하시오. 

SELECT EMPNO 사원명, SAL 급여, (SAL*12) + 100 연간총수입
    FROM EMP
    ORDER BY (SAL*12) + 100 DESC;
    
<6번> 급여가 500을 넘는 사원의 이름과 급여를 표현, 급여가 많은 것부터 작은 순으로 출력하시오.
SELECT ENAME 사원명, SAL 급여
    FROM EMP
    WHERE SAL>=500
    ORDER BY SAL DESC;

<7번> 사원번호가 7788인 사원의 이름과 부서번호를 출력하시오.

SELECT A.ENAME 사원명, A.DEPTNO 부서번호, B.DNAME 부서명, A.EMPNO 사원번호
    FROM EMP A, DEPT B
    WHERE A.EMPNO = '7788' AND A.DEPTNO = B.DEPTNO; --AND A.DEPTNO = B.DEPTNO; 이거 안쓰면 안됨
    
    
<8번> 급여가 400에서 500 사이에 포함되지 않는 사원의 이름과 급여를 출력하시오.
SELECT ENAME 사원명, SAL 급여 --방법1
    FROM EMP
    WHERE SAL<400 OR SAL >500;

SELECT ENAME 사원명, SAL 급여 --방법2
    FROM EMP
...
<34번>
INNER JOIN과 USING 연산자를 사용하여10번 부서에 속하는
모든 담당 업무의 고유 목록(한 번씩만 표시)을 부서의 지역명을 포함하여 출력 하시오.
...
<35번>
NATURAL JOIN을 사용하여
커미션을 받는 모든 사원의 이름, 부서이름, 지역 명을 출력하시오.

SELECT ENAME 사원이름, DNAME 부서이름, LOC 지역명, EMP.COMM 커미션
FROM EMP NATURAL JOIN DEPT
WHERE COMM IS NOT NULL;

<36번>
EQUI 조인과 WildCARD를 사용하여 이름에 ‘민’가 포함된 모든 사원의 이름과 부서명을 출력하시오.

SELECT ENAME 사원명 , DNAME 부서명
FROM EMP , DEPT
WHERE EMP.DEPTNO = DEPT.DEPTNO AND ENAME LIKE '%민%';

<37번>
NATUAL JOIN을 이용하여 대에 근무하는 모든 사원의 이름, 업무, 부서번호 및 부서명을 출력하시오.

SELECT ENAME 사원명, DEPTNO 부서번호, DNAME 부서명, LOC 근무지
FROM DEPT NATURAL JOIN EMP
WHERE LOC = '대전'
ORDER BY DEPTNO;

<38번> SELF JOIN을 사용하여 사원의 이름 및 사원번호를 관리자 번호와 함께 출력하시오.

SELECT e1.ENAME 사원이름, e1.EMPNO 사원번호, e1.MGR 관리자번호, e2.ENAME 관리자성명
FROM EMP e1, EMP e2
WHERE e1.MGR = e2.EMPNO;

select * from emp;

<39번> SELF JOIN을 사용하여 지정한 사원의 이름,부서번호, 지정한 사원과 동일한 부서에서 근무하는
사원을 출력하시오(김동혁)

SELECT e2.ENAME 사원명, e2.DEPTNO 부서번호
FROM EMP e1, EMP e2
WHERE e1.DEPTNO = e2.DEPTNO AND e1.ENAME = '김동혁';

<40번> 사원번호가 7934인 사원과 담당업무가 같은 사원을 표시(사원 이름과 담당업무)하시오,

SELECT e1.EMPNO 사원번호, e1.JOB 담당업무, e1.ENAME 사원이름, e2.ENAME 사원이름2
FROM EMP e1, EMP e2 
WHERE e1.JOB = e2.JOB
    AND e1.EMPNO = '7934';
    
<40-1번> 상품테이블에서 상품코드, 상품명, 분류명, 거래처 명을 조회하시오.
